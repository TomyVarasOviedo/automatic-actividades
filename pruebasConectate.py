...
def  inciar_sesion() -> dict:
    ...
    if "InscripcionActividadesExtracurriculares.aspx" in response_post.url:
        ...
        if response.status_code == 200:
            ...
            print(f"Error al obtener la respuesta: {response.status_code} -> {response.text}")
...
def indentificar_actividades(data:list) -> list:
    ...
    for actividad in data:
        ...
        if actividad['CupoDisponible'] == 0:
            continue
        if actividad['HabilitadoInscripcion'] == False or actividad['Cancelado'] or actividad['Cerrada']:
            continue
        if not "https://" in actividad['Lugar']:
            continue
        actividades.append(
            Actividades(
                actividad['Id'],
